...
def _read_rpc_port(default_port):
    for env_name in ("AIRSIM_PORT", "AIRSIM_RPC_PORT", "DRV_UNREAL_RPC_PORT"):
        ...
        try:
            ...
            logging.warning("Ignoring invalid %s value for AirSim RPC port: %s", env_name, env_value)
    return default_port


def resolve_rpc_endpoint(ip="", port=41451):
    if ip == "":
        ip = os.getenv("AIRSIM_HOST", "host.docker.internal")
        port = _read_rpc_port(port)
    return ip, port


class VehicleClient:
    def __init__(self, ip="", port=41451, timeout_value=3600):
        ip, port = resolve_rpc_endpoint(ip, port)
        self.ip = ip
        self.port = port
