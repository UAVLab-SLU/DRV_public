...
const frontendHost =
  process.env.FRONTEND_HOST || process.env.REACT_APP_FRONTEND_HOST || 'localhost';
      ...
      on('task', {
        verifyLocalMockReportFromTimestamp({ uiTimestamp }) {
          ...
          if (!fs.existsSync(reportsRoot))
            return { found: false, reason: 'reports folder not found' };
          ...
          if (!fs.existsSync(reportPath))
            return { found: false, reason: 'mock_report.txt not found', reportPath };
