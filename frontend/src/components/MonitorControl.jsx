...
function buildMonitorState(monitors) {
  ...
  Object.entries(monitors).forEach(([monitorId, monitorConfig]) => {
    if (
      nextMonitorState[monitorId] == null ||
      monitorConfig == null ||
...
export default function MonitorControl(monJson) {
  ...
  const [monitor, setMonitor] = React.useState(() =>
    buildMonitorState(monJson.mainJsonValue.monitors),
  );

  const [envConf, setEnvConf] = React.useState(
    monJson.mainJsonValue.environment != null
      ? monJson.mainJsonValue.environment
      : {
          enableFuzzy: false,
  ...
  const singleMonitors = [
        ...
        <Grid container direction='row'>
          ...
          <FormGroup>
            <FormControlLabel
              control={
                <Switch
                  checked={monitor.collision_monitor.enable}
                  onChange={(e) => handleChangeSwitch(e, 'collision_monitor')}
                  inputProps={{ 'aria-label': 'controlled' }}
                />
              }
              label={monitor.collision_monitor.enable ? 'Enabled' : 'Disabled'}
            />
          </FormGroup>
        </Grid>
      ),
    ...
    },
    {
        ...
        <Grid container direction='row'>
          ...
          <FormGroup>
            <FormControlLabel
              control={
                <Switch
                  checked={monitor.landspace_monitor.enable}
                  onChange={(e) => handleChangeSwitch(e, 'landspace_monitor')}
                  inputProps={{ 'aria-label': 'controlled' }}
                />
              }
              label={monitor.landspace_monitor.enable ? 'Enabled' : 'Disabled'}
            />
          </FormGroup>
        </Grid>
      ),
      ...
      tableData: (
        <React.Fragment>
          {monitor.landspace_monitor.enable === true ? (
            <>
              <Grid item xs={12}>
                <strong>Configure safe landing spots for drones </strong>
              </Grid>
              <MonitorTabels
                hideAltitude='true'
                errorMessage='false'
                jsonVal={setLandspaceParameters}
              />
            </>
          ) : null}
        </React.Fragment>
    ...
    },
    {
      ...
      btns: (
        <React.Fragment>
          {monitor.point_deviation_monitor.enable == true ? (
            <Grid item xs={12}>
              <Grid item xs={12} style={{ paddingBottom: 20 }}>
                <strong>Configure the acceptable flight drift </strong>
              </Grid>
              <FormGroup>
                <Tooltip
                  title='Max total acceptable deviation from the actual flight path (in meters)'
                  placement='bottom'
                >
                  <Grid item xs={6}>
                    <TextField
                      id='standard-basic'
                      label='Deviation/Drift (in meters)'
                      type='number'
                      step='0.1'
                      variant='standard'
                      onChange={handleDeviationPercent}
                      value={monitor.point_deviation_monitor.param[0]}
                    ></TextField>
                  </Grid>
                </Tooltip>
              </FormGroup>
              <Grid item xs={12} style={{ paddingTop: 50 }}>
                <Alert severity='info'>
                  Note: Flight drift is calculated by analyzing whether the sUAS or drone drifts
                  away from the planned path and breaches the configured distace at any point in the
                  mission
                </Alert>
              </Grid>
            </Grid>
      ...
      enableBtn: (
        <Grid container direction='row'>
          ...
          <FormControlLabel
            control={
              <Switch
                checked={monitor.point_deviation_monitor.enable}
                onChange={(e) => {
            ...
            label={monitor.point_deviation_monitor.enable ? 'Enabled' : 'Disabled'}
          />
        </Grid>
      ),
      tableData: null,
    ...
    },
    {
        ...
        <Grid container direction='row'>
          ...
          <FormGroup>
            <FormControlLabel
              control={
                <Switch
                  checked={monitor.no_fly_zone_monitor.enable}
                  onChange={(e) => handleChangeSwitch(e, 'no_fly_zone_monitor')}
                  inputProps={{ 'aria-label': 'controlled' }}
                />
              }
              label={monitor.no_fly_zone_monitor.enable ? 'Enabled' : 'Disabled'}
            />
          </FormGroup>
        </Grid>
      ),
      mutlipleBtn: (
        <React.Fragment>
          {monitor.no_fly_zone_monitor.enable == true ? (
            <>
              <Grid item xs={12}>
                <strong>Configure no-fly zones : Ferature Development in Progress </strong>
              </Grid>
              <img src='../images/dummy_map_configurator.png'></img>
              <Alert severity='info'>
                Note : We are excited to announce that we are currently developing a new feature
                that will allow users to create 3D polygons on the map to define one or more no fly
                zones. This feature will give users greater control over where their drones can and
                cannot fly, helping to ensure safer and more responsible drone operation.{' '}
              </Alert>
    ...
    },
    {
      ...
      description:
        'Test whether the drones breach the minimum separation distance with other drones',
      btns: (
        <React.Fragment>
          {monitor.min_sep_dist_monitor.enable == true ? (
            <React.Fragment>
              <Grid item xs={12}>
                <strong>Configure minimum separation distances </strong>
              </Grid>

              <Tooltip
                title='Enter Minimum Horizontal separation distance in Meters'
                placement='bottom'
              >
                <Grid item xs={6}>
                  <TextField
                    id='standard-basic'
                    label='Minimum Horizontal Separation (meters)'
                    type='number'
                    step='0.1'
                    variant='standard'
                    style={{ width: 250 }}
                    onChange={handleMinHorizontalSepChange}
                    value={monitor.min_sep_dist_monitor.param[0]}
                  ></TextField>
                </Grid>
              </Tooltip>
            </React.Fragment>
        ...
        <Grid container direction='row'>
          ...
          <FormGroup>
            <FormControlLabel
              control={
                <Switch
                  checked={monitor.min_sep_dist_monitor.enable}
                  onChange={(e) => handleChangeSwitch(e, 'min_sep_dist_monitor')}
                  inputProps={{ 'aria-label': 'controlled' }}
                />
              }
              label={monitor.min_sep_dist_monitor.enable ? 'Enabled' : 'Disabled'}
            />
          </FormGroup>
        </Grid>
      ),
    ...
    },
    {
      ...
      btns: (
        <React.Fragment>
          {monitor.battery_monitor.enable == true ? (
            <Grid item xs={12}>
              <Grid item xs={12} style={{ paddingBottom: 20 }}>
                <strong>Configure the current battery capacity </strong>
              </Grid>
              <FormGroup>
                <Tooltip
                  title='Input the current battery capacity of the drone as % (0-100)'
                  placement='bottom'
                >
                  <Grid item xs={6} style={{ marginBottom: '20px' }}>
                    <TextField
                      ...
                      onChange={(val) => handleBatteryMonitor(val, 0)}
                      value={monitor.battery_monitor.param[0]}
                    ></TextField>
                  </Grid>
                </Tooltip>
                <Tooltip
                  title='Input the target failure battery percentage for testing'
                  placement='bottom'
                >
                  <Grid item xs={6}>
                    <TextField
                      id='target-failure'
                      label='Target Failure Percentage (%)'
                      type='number'
                      step='0.1'
                      variant='standard'
                      style={{ width: '190px' }}
                      inputProps={{ min: 0, max: 100 }}
                      onChange={(val) => handleBatteryMonitor(val, 1)}
                      value={monitor.battery_monitor.param[1]}
                    ></TextField>
                  </Grid>
                </Tooltip>
              </FormGroup>
      ...
      enableBtn: (
        <Grid container direction='row'>
          ...
          <FormControlLabel
            control={
              <Switch
                checked={monitor.battery_monitor.enable}
                onChange={(e) => {
            ...
            label={monitor.battery_monitor.enable ? 'Enabled' : 'Disabled'}
          />
        </Grid>
      ),
      tableData: null,
    ...
    return (
      <div>
        <div>
          ...
          <p style={{ fontSize: '12px', fontStyle: 'oblique' }}>
            Check the box if you would like there to be a Fuzzy Test for the time of day.
          </p>
        </div>
        ...
        <div>
          ...
          <p style={{ fontSize: '12px', fontStyle: 'oblique' }}>
            Check the box if you would like there to be a Fuzzy Test for the drone position.
          </p>
        </div>
        ...
        <div>
          ...
          <p style={{ fontSize: '12px', fontStyle: 'oblique' }}>
            {' '}
            Check the box if you would like there to be a Fuzzy Test for the wind.{' '}
          </p>
        </div>
      </div>
    );
                                    ...
                                    return (
                                      <div key={i}>
                                        ...
                                        >
                                          ...
                                          >
                                            <Button style={{ color: 'var(--dw-color-error)' }}>
                                              Delete Zone
                                            </Button>
                                          </ButtonGroup>
                                          <div style={{ paddingTop: '10px' }}>
                                            <MonitorTabels
                                              hideAltitude='false'
                                              errorMessage='true'
                                              jsonVal={(e) => setNoFlyParameters(e, i)}
                                            />
                                          </div>
                                        </Typography>
                                      </div>
