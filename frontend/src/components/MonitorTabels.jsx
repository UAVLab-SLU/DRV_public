...
export default function MonitorTabels(colData) {
  const useStyles = makeStyles({
    ...
    tableRow: {
      border: '1px solid var(--dw-color-error)',
      display: 'table-row',
    },
  });
  ...
  const addRowStyle = {
    display: 'block',
    background: 'var(--dw-color-table-row-add-bg)',
    width: '100%',
  };
  const [style, setStyle] = React.useState(addRowStyle);
  const addNewRow = () => {
  ...
  return (
    <Paper className={classes.root}>
      {colData.errorMessage == 'true' ? (
        <div style={{ padding: '5px', color: 'var(--dw-color-text-link)' }}>
          {rows.length < 4 ? (
          ...
          <TableHead>
            <TableRow>
              {colData.hideAltitude == 'true'
                ? columnsWithoutAltitude.map((column) => (
                    <TableCell
                      key={column.id}
                      align={column.align}
          ...
          <TableBody>
            {rows.map((row, index) => (
              <TableRow key={index}>
                ...
                {colData.hideAltitude == 'true' ? null : (
                  <TableCell>
                    <TextField
                      id='Altitude'
                      variant='standard'
